...
*merge m:1 hhid codpr using "$presim/08_subsidies_elect.dta", keepusing(consumption_electricite type_client prepaid_woyofal) nogen
...
egen subsidy_elec_direct = rowtotal(subsidy1 subsidy2 subsidy3 subsidy4 subsidy5 subsidy6 subsidy7) 
...
gen sub_gasoline = c_gasoline * ${sub_gasoline} / 100
gen sub_gasoil = c_gasoil * ${sub_gasoil} / 100
gen sub_lpg = c_lpg * ${sub_lpg} / 100

egen sub_fuel = rowtotal(sub_gasoline sub_gasoil sub_lpg)

rename sub_fuel subsidy_fuel_direct
rename sub_gasoline subsidy_f1_direct
rename sub_gasoil subsidy_f2_direct
rename sub_lpg subsidy_f3_direct
...
keep hhid codpr informal_purchase sector pourcentage achats_net achats_net_excise achat_gross 
...
replace subsidy_elec_direct = subsidy_elec_direct * pourcentage * informal_purchase
...
collapse (sum) subsidy_elec_direct subsidy_elec_indirect subsidy_eau_direct subsidy_eau_indirect (mean) subsidy_emel_direct subsidy_emel_indirect subsidy_inag_direct subsidy_inag_indirect subsidy_fuel_direct subsidy_fuel_indirect subsidy_f1_direct subsidy_f2_direct subsidy_f3_direct, by(hhid) 
