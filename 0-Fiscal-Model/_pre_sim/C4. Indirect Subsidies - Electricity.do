...
* Test consumption: All HH are social users
    gen kwh_test = (($purchases - (`prime_fix_s' * 12) - (`redevance' * 12)) / `tarif_s' ) / 12 if $purchases > 0 // this includes VAT assuming everybody pay it  

	replace kwh_test = 0 if kwh_test < 0
...
* Get annual kWh
	gen kwh = ($purchases - (prime_fix * 12) - (`redevance' * 12)) / (tarif) if $purchases > 0 // Assumes everybody pay VAT, underestimate the consumption of Kwh
...
keep hhid codpr type_client consumption_electricite prepaid_woyofal hh_elec* codpr_elec
