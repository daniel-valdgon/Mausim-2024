...
* Standardize / Filter
keep hhid A1 A2 A3 A5 A6 A10*

* Save
*save "$presim/Temp/grape.dta", replace
tempfile grape 
save `grape', replace 

/* Raw Electricity expenditure
use "$data_sn/Datain/retro2_2019.dta", clear
...
* Standardize / Filter
keep if  J3_0 == 11
keep hhid J3_1 J3_2B J3_2A J3_3
gunique hhid

*tab1 J3_1 J3_2B J3_2A J3_3
...
/* Articles
use "$data_sn/Datain/Equipement_2019.dta", clear // F
...
label list
label drop labels1
replace F16A = 0 if F16A == 2

tab F16_CODE, nol

ren F16A art_

keep hhid F16_N art_ max_G7 // F16B

* Reshape 
reshape wide art_, i(hhid max_G7) j(F16_N)
...
*merge 1:1 hhid using `articles', nogen

*merge m:1 hhid using `electricity', nogen

* Standardize / Filter
drop hhsize 

tab codpr if coicop == 4
tab codpr if coicop == 4, nol
keep if codpr == 376
...
* Check // Variables changed their label, to fix...
tab uno [iw = pondih] // Total population
tab I6 [iw = pondih], m // The village has electricity?
tab I8 [iw = pondih], m // The household utilices the electricity?
tab I9 [iw = pondih], m // Are you satisfied?
tab I10 [iw = pondih], m
*tab max_G7 [iw = pondih], m

tab I10_1 I8 [iw = pondih], m
tab I10_1 I8 [iw = pondih] if depan > 0, m
*tab I10_1 J3_2B [iw = pondih], m row // Bi-monthly

* Final Variables
gen hh_elec = .
replace hh_elec = 1 if I8 == 1
replace hh_elec = 0 if I8 == 2
replace hh_elec = 0 if I8 == .


* Allocate social and domestic
tab A5 hh_elec [iw = pondih], m col

tab A3 hh_elec [iw = pondih], m nofreq row
tab A3 hh_elec [iw = pondih], m nofreq row nol

tab A3 hh_elec [iw = pondih], m nol

* 52% Social, 42% domestic

* Option 1
* Rule, if the commune (A3) has more than 90% of the population with energy, the households in that commune are allocated to domestic, if the commune has less than 10% of the population with energy, the households in that commune are allocated to social

gen domestic = .
replace domestic = 1 if inlist(A3, 8101, 11201, 11301, 12101, 13101, 13201, 13501, 13601, 13701, 13801, 13901)

replace domestic = 0 if domestic == .
replace domestic = . if hh_elec == 0

tab domestic [iw = pondih], nol freq
tabstat depan [aw = pondih], s(p50 mean sum) by(domestic)

* Option 2
* Asignacion social y domestique

*egen tot_depan_yes = total(depan*hhweight*hh_elec), by(A3)
*egen tot_depan_no = total(depan*hhweight*(1-hh_elec)), by(A3)
...
sum depan, d
gen condition = 1 if depan > 0 & inlist(I10_1, 1) & depan < r(p99)

gen id = _n if condition
gen random = runiform() if condition

sort random
sum id
gen user = 1 if id <= r(N) * $per
replace user = 0 if user==.
drop id random

gsort hhid

tab I10_1 user  [iw = pondih], m
*/

* Yearly Expenditure
*gen sum_depan = depan * hhweight / 1000000

*sum sum_depan, d

tabstat depan [aw = pondih], s(p50 mean sum) by(hh_elec)

*tabstat depan [aw = pondih], s(p1 p10 p25 p50 p75 p90 p99 mean sum) by(I10_1)

tabstat depan [aw = pondih], s(p1 p10 p25 p50 p75 p90 p99 mean sum) by(domestic)

*gen depan_elec = depan*(1-0.16)


* Yearly consumption kWh
* Parameters
global tarif_s = 24.6 // 2.46 MRU
global prime_fix_s = 279.9

global tarif_d = 59 // 5.9 MRU
global prime_fix_d = 1650.7

global redevance = 404
...
* kWh
gen kwh = (depan - (prime_fix * 12) - ($redevance * 12)) / tarif

mvencode kwh, mv(0)

* Bi-monthly consumption kWh
gen kwh_bi = round(kwh / 6, 1)

* Monthly consumption kWh
gen kwh_m = round(kwh / 12, 1)

tabstat kwh_m [aw = pondih] if domestic == 1, s(p50 mean sum) by(A3)
tabstat kwh_m [aw = pondih] if domestic == 0, s(p50 mean sum) by(A3)

tabstat depan kwh_m [aw = pondih], s(p50 mean sum) by(domestic)


br hhid codpr depan I8 I10_1 J* *kwh*

gen type_client = 1 if I8==1 & I10_1==1
mvencode type_client, mv(0)
...
* hhid
* consumption_electricite (Bi-monthly electricity consumption / kWh)
* type_client (1, 2, 3, .) ()
* prepaid_woyofal (1, 0, .)

gab

keep hhid m* bi* type_client prepaid_woyofal depan
...
tab m_kwh_d [iw = pondih], m

gen r_m_kwh_d = . 
replace r_m_kwh_d = 1 if inrange(m_kwh_d, 0, 5)
replace r_m_kwh_d = 2 if inrange(m_kwh_d, 5, 10)
replace r_m_kwh_d = 3 if inrange(m_kwh_d, 10, 15)
replace r_m_kwh_d = 4 if inrange(m_kwh_d, 15, 20)
replace r_m_kwh_d = 5 if inrange(m_kwh_d, 20, 30)
replace r_m_kwh_d = 6 if inrange(m_kwh_d, 30, 40)
replace r_m_kwh_d = 7 if inrange(m_kwh_d, 40, 50)
replace r_m_kwh_d = 8 if inrange(m_kwh_d, 50, 60)
replace r_m_kwh_d = 9 if inrange(m_kwh_d, 60, 75)
replace r_m_kwh_d = 10 if inrange(m_kwh_d, 75, 100)
replace r_m_kwh_d = 11 if inrange(m_kwh_d, 100, 200)
replace r_m_kwh_d = 12 if inrange(m_kwh_d, 200, 300)
replace r_m_kwh_d = 13 if inrange(m_kwh_d, 300, 400)
replace r_m_kwh_d = 14 if inrange(m_kwh_d, 400, 500)
replace r_m_kwh_d = 15 if inrange(m_kwh_d, 500, .)

tab r_m_kwh_d [iw = pondih], m
tab r_m_kwh_d [iw = pondih] if r_m_kwh_d !=1, m
...
global price2_DMP=101.64*1.025 
...
merge 1:1 hhid using "$data_sn/s11_me_SEN2018.dta", nogen keepusing(s11q37a s11q37b s11q36 s11q34) // Added Gabriel keepusing
