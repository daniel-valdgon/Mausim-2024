...
if "`c(username)'"=="gabriellombomoreno" {
	...
	global xls_out    	"${report}/Fs_Specific.xlsx"
...
* HH size
tabstat hhsize [aw = hhweight], s(min p25 p50 p75 max mean) by(yd_deciles_pc)

* Bimonthly consumption_electricite
tabstat consumption_electricite [aw = hhweight], s(min p25 p50 p75 max mean) by(yd_deciles_pc)
...
merge m:1 hhid using "$data_out/output_${proj_`scenario'}.dta", nogen
