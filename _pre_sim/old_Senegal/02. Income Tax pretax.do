...
foreach r in 1 2 3{
	global tholds "0"
	global rates  ""
	foreach t of global tholdsRGU`r' {
		local rate = ${RGU`r'rate`t'}*100
		global rates  "$rates `rate'"
		global tholds "$tholds ${RGU`r'max`t'}"
		global maxthold = ${RGU`r'max`t'}
	}
	global rates  "$rates `rate'"
	dirtax inclab if regime_g ==`r', rates($rates ) tholds($tholds ) net gen(inc_selfemp_pretax`r')
	replace inc_selfemp_pretax`r'=. if inc_selfemp_pretax`r'>$maxthold
	replace inc_selfemp_pretax`r'=inclab+${RGU`r'_floor} if inc_selfemp_pretax`r'-inclab<${RGU`r'_floor}
}
egen double inc_selfemp_pretax = rowtotal(inc_selfemp_pretax*), missing

gen double inctax_self=inc_selfemp_pretax-inclab
