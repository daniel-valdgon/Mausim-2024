...
*merge n:1 hhid using `income_enterprise', gen(income_enterprise)
...
recode s04q45 s04q47 (9999=0)

gen double impap=s04q45 if s04q45_unite==2 //(4.45)	 A combien évaluez-vous les primes ( uniquement celles qui ne sont pas incluses dans le salaire)?  (Other payments)
replace impap=s04q45*4 if s04q45_unite==1
replace impap=s04q45/3 if s04q45_unite==3
replace impap=s04q45/12 if s04q45_unite==4
...
egen impa_f=rowtotal(impa impap impaes) //AGV: I included impap, it was not considered before
...
gen double isa=s04q58 if s04q58_unite==2
replace isa=s04q58*4 if s04q58_unite==1
replace isa=s04q58/3 if s04q58_unite==3
replace isa=s04q58/12 if s04q58_unite==4

recode s04q60 s04q62 (9999=0)

gen double isap=s04q60 if s04q60_unite==2
replace isap=s04q60*4 if s04q60_unite==1
replace isap=s04q60/3 if s04q60_unite==3
replace isap=s04q60/12 if s04q60_unite==4

gen double isaes=s04q62 if s04q62_unite==2
replace isaes=s04q62*4 if s04q62_unite==1
replace isaes=s04q62/3 if s04q62_unite==3
replace isaes=s04q62/12 if s04q62_unite==4
...
if $devmode == 1{
	...
	graph bar sinclab [aw=hhweight], over(ndtet) ytitle(Mean of labor income)
	...
	twoway (hist lincl1 [fw=hhweight] if lincl1!=0, freq) (hist lincent [fw=hhweight] if lincent!=0, color(stc2%50) freq), xlabel(6.9 "1K" 9.2 "10K" 11.5 "100K" 13.8 "1M" 16.1 "10M" 18.4 "100M" 18.4 "100M" 20.72 "1B") legend(label(1 "Wage") label(2 "Entrepr.") ring(0) pos(10))
	
	global lnvals = ""
	foreach var in -1000000 -10000 -100 -10 0 10 100 10000 1000000 1000000000{
		*local lnv = round(ln(`var'),0.001)
		local lnv = round(ln(`var'+sqrt((`var')^2 +1)),0.001)
		global lnvals `"$lnvals `lnv' "`var'" "'
	}
	twoway (hist sincl1 [fw=hhweight], freq) (hist sincent [fw=hhweight], color(stc2%50) freq), xlabel($lnvals ) legend(label(1 "Wage") label(2 "Entrepr.") ring(0) pos(10))
	...
	twoway (hist sinc_cons [fw=hhweight], freq) (hist sdt [fw=hhweight], color(stc2%50) width(.56867543) freq), xlabel($lnvals ) legend(label(1 "Total consumable income") label(2 "Total consumption") ring(0) pos(10))
	
	drop ldt linc sinclab
}
